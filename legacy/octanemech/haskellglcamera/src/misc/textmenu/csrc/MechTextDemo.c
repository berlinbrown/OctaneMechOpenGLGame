...
void setTextColor(TextBoxPtr text, 
...
void formatStrCat(TextBoxPtr text, char *str) {
  ...
  while (*cP != '\0') {
	...
	while(cP > buff && *cP != ' ') cP--;
      ...
      *cP = '\0';
      // Eliminate Space
      if (text->mode & TEXT_WRAP_SPACES && buff2[0] == ' ') cP2 = &buff2[1];
      else cP2 = buff2;
      strcat(cP, cP2);
      text->currX = text->boxL;
    } 
...
void renderString(TextBoxPtr text, int x, int y, int maxFlag, 
		  int maxY, char* string) {
...
void renderStaticText(TextBoxPtr text) {
  ...
  renderString(text, text->boxL, text->boxT, 1, text->boxB, 
	  text->drawBufferPtr);
  popTextEnd();
